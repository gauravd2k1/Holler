...
/// `(id, kind, name)` for the devices this seed creates.
///
/// **THE CLOUD GETS THESE ROWS TOO, AND WITHOUT A CREDENTIAL (D11).** The till
/// minted its `device` row straight into edge SQLite and nothing ever
/// registered it with the backend, so every till-authored order replayed to a
/// cloud with no row to join to: the scenario board measured four orders whose
/// `device_id` resolves to nothing (S-SYNC-13), and the admin Orders screen
/// cannot name who took an order.
///
/// `device` is cloud-owned identity — enrollment is a cloud route — and the
/// schema says in as many words that a row may exist UNENROLLED ("an admin
/// registered it ahead of install; it simply cannot sync until it holds a
/// credential", postgres/0008). That is exactly this case, so the cloud seeder
/// writes the row with `enrolled_at` NULL and **seeds no `device_credential`
/// at all**. A seeded row grants nothing: the plaintext token is returned once
/// at enrollment and there is no route that reads it back.
///
/// The WAITER device is deliberately absent. It is created by a real
/// enrollment at `demo-up.ps1` step 7, and seeding a row for it would put a
/// second, unenrolled row under the same `(outlet_id, name)` unique index.
fn seed_devices() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        (DEVICE_ID, "POS", "Dev Till 1"),
        (KDS_DEVICE_ID, "KDS", "Dev Kitchen Screen 1"),
    ]
}
...
fn build_shared_catalogue(identity: &OutletIdentity) -> Result<Value, String> {
    ...
    // ---- the devices this outlet runs on (D11) ----
    // Cloud-owned identity, seeded UNENROLLED and with no credential: the
    // cloud needs the row so a replayed order can be attributed, and a row
    // without a credential can do nothing.
    let devices: Vec<Value> = seed_devices()
        .into_iter()
        .map(|(id, kind, name)| {
            json!({ "id": id, "outlet_id": OUTLET_ID, "kind": kind, "name": name })
        })
        .collect();
    ...
    Ok(json!({
        ...
        // CLOUD-OWNED IDENTITY, SEEDED WITHOUT A CREDENTIAL (D11). The till's
        // row existed only at the edge, so till-authored orders replayed to a
        // cloud that had nothing to join them to and the admin Orders screen
        // could not name the device. enrolled_at stays NULL and no
        // device_credential is seeded: postgres/0008 says a row may exist
        // unenrolled, and an unenrolled row grants nothing.
        "devices": devices,
...
fn seed(
    ...
    for (id, kind, name) in seed_devices() {
        repo::upsert_device(
            conn,
            &Device {
                id: id.to_string(),
