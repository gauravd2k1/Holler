...
- **Cross-tenant menu isolation rests on per-query convention, not the database. Named milestone: M8 (multi-outlet), and no later.** Confirmed 2026-08-20: there is **no** `ROW LEVEL SECURITY` and **no** `CREATE POLICY` anywhere in `packages/contracts/postgres/`. Tenant isolation for the whole menu tree — `menu_item`, `menu_item_variant`, `menu_item_modifier`, and now `recipe`, `recipe_ingredient`, `modifier_ingredient_delta` — is a join every query must remember to write. A forgotten join is a cross-tenant menu leak, and nothing in the schema stops it. This is why `recipe` deliberately does **not** carry its own `tenant_id` (ADR-018 §2): a lone tenant column that no constraint forces to agree with the tree is a second answer to "whose row is this", which is worse than one answer reached by a join. The fix is RLS on the tree, not a column on one table. It must land before multi-outlet, where several tenants' data is routinely in one query's blast radius.

- **Two more unenforced `APPEND-ONLY` claims, found by the new lint.** `audit_event` (`sqlite/0002:63`, "Local append-only audit") and `cash_movement` (`sqlite/0006:392`, "Append-only: a correction is another movement") each assert immutability with no trigger behind it. **`audit_event` is the serious one** — an audit log that can be edited is not an audit log. Both are held by `every_append_only_claim_has_a_trigger_behind_it` (`edge/database/src/migrations.rs`), which fails if a new claim of this shape appears and fails if these are fixed without removing the entry. Not bundled into an inventory milestone. Separately, `invoice`'s claim is a **wording** defect rather than a missing trigger: the row has a mutable `ISSUED→CANCELLED` status, so `sqlite/0006:176` should say "append-only replay" as its PostgreSQL twin already does.

- ~~**PostgreSQL `payment` has no append-only enforcement**~~ — **FIXED at contracts 0.5.0, `postgres/0018_payment_append_only_triggers.sql`.** Kept here because the fix does not retire the record: this was the **second** M3 defect found during M4 planning, alongside the UTC business-date bucketing, and M3 was reported complete carrying both. Original entry: **only a comment saying it is.** Found 2026-08-20 while declaring the store asymmetries. `sqlite/0009_payment_append_only_triggers.sql` installs real `BEFORE UPDATE`/`BEFORE DELETE` triggers; `postgres/0007_m3_billing.sql` carries the words "APPEND-ONLY" at line 286 and nothing behind them. So the guarantee ADR-016 relies on is structurally enforced at the edge and *documented* in the cloud — and the cloud is where a support engineer with a psql prompt actually goes. Contracts 0.5.0's `stock_ledger_entry` does not repeat this: it ships the trigger in both stores. **Trigger: the next migration touching `payment`, or any incident involving a corrected tender.** The fix is the same plpgsql trigger pattern now in `postgres/0016_m4_stock_ledger.sql`.
