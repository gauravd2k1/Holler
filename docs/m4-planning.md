...
| **T4** | `backend/internal/inventory` — config write routes, `/sync/config` contribution, envelope-wrapped ledger ingest, **cross-tenant isolation tests for `recipe`, `recipe_ingredient`, `modifier_ingredient_delta`**, **the `printer_roles` sync-bundle fix**, and **a guard asserting every config aggregate the edge needs appears in `syncConfigResponse`** | Directory is currently empty. The isolation tests are not optional: `backend/internal/menu` has none today, and adding three tables to an untested boundary is how it stays untested. The retrofit for the pre-existing menu tables stays in the backlog; M4's own tables do not inherit that exemption. |
