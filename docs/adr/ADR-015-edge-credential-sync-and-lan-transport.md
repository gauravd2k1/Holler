...
The frozen schema's own description already claims delivery is *"only over TLS, only to an enrolled edge node."* Two thirds of that sentence is currently aspirational. **This ADR does not fix it, and the mirrors landing here do not make it true.** Device enrollment carries a hard trigger in `docs/backlog-m2.md`: it blocks any pilot deployment, including a single outlet. Shipping credential sync to a real restaurant before enrollment exists would be a straightforward security failure, and naming it here is the point of the entry.
