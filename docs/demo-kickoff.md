...
1. Seed parity. One seed source loads identical menu, variants, recipes, inventory, supplier into BOTH cloud and edge. **Landed 2026-09-13, and extended: `seed/outlet.toml` is now the single source for WHO the restaurant is too — name, legal entity, address, GSTIN, FSSAI, invoice prefix, bill footer, timezone, business-day start, UPI payee, logo. Gitignored, per-installation, no default; every runner refuses without it. Onboarding a restaurant is writing one file, never editing code. Evidence in `docs/demo-status.md`.** Ruling: P2/P3/P5 are fixed, so the M5 prohibition on seeding the cloud is lifted. Bootstrap the edge from this clean seed (0035 applies here). Assert after seeding: zero blocked rows, zero deduction gaps, banner empty.
