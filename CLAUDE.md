...
- **EVERY REPORT OPENS WITH THE CI STATUS OF HEAD, AND A HANDOFF MAY NOT CLAIM A CLEAN TREE WHILE CI IS RED.** One line, from `gh run list --limit 1`: the sha and the conclusion. **LOCAL GREEN IS NOT GREEN.** On 2026-09-16 a session ran twelve commits, reported named passing checks for each, and never once looked at CI — which had failed on **every run in the visible history, zero successes in forty**. One of those failures was four `crash_durability` tests that could not run at all on a clean checkout, and it hid a real durability defect underneath it: an aborted receipt consuming a GRN number. `gh` is authenticated here and the check costs one command. A green local suite says the code works on the machine that wrote it; CI is the only thing that says it works on a checkout.
