...
func main() {
	// NO DEFAULT, AND THE DEFAULT IT USED TO HAVE NAMED THE LIVE DATABASE.
	//
	// This fell back to ...localhost:5432/holler, so a run that named no
	// database seeded the operator's own. That is D13's hazard exactly — a
	// scratch target named in one argument that the step doing the work never
	// reads — and it is how CI's own migrate step broke silently: the
	// workflow's database was renamed to holler_scratch_ci and this step went
	// on connecting to a `holler` that no longer existed, because it reads
	// DATABASE_URL and nothing set it.
	//
	// NO VARIABLE, NO RUN. A missing connection string is now a refusal with
	// the reason, not a guess at which database was meant.
	databaseURL := flag.String("database-url", os.Getenv("DATABASE_URL"),
		"PostgreSQL connection string (or DATABASE_URL). REQUIRED: there is no default, "+
			"because the default used to name the live `holler` database")
	...
	if strings.TrimSpace(*databaseURL) == "" {
		log.Fatal("devseed: no database named. Set DATABASE_URL or pass --database-url. " +
			"There is deliberately no default: the one this used to have named the live " +
			"`holler` database, so a run that named nothing seeded the operator's own.")
	}
