#!/usr/bin/env python3
"""Generate the client menu half of the demo seed from the client's own workbook.
...
reads every row with `include = Y` and writes two generated artefacts:
  ...
  edge/database/src/bin/devseed/client_menu.rs  the categories/items/variants and
                                       modifiers as a Rust literal, consumed by devseed
  seed/menu-manifest.json         counts plus a digest of both sides
...
`seed/demo-outlet.json` from the Rust data as before, so the chain is
    ...
    gong_menu.xlsx -> devseed/client_menu.rs -> demo-outlet.json -> {SQLite, Postgres}

and every link but the first is checked by something that fails when it drifts:
`devseed`'s own `client_menu_matches_the_generated_manifest` test covers the
...
# The client's own workbook. A PATH, NOT A CONSTANT BURIED IN CODE: Holler is
# white-label, so the next restaurant arrives with a differently-named file and
# must not need a code edit.
#
#     python scripts/menu-to-seed.py --workbook path/to/their-menu.xlsx
#
# The default is whatever the current client sent, and the manifest records the
# path and the file's digest, so a regeneration from a different workbook is
# visible in the diff rather than silent.
DEFAULT_WORKBOOK = REPO_ROOT / "menu_imgs_gong" / "gong_menu.xlsx"
WORKBOOK = DEFAULT_WORKBOOK
RUST_OUT = REPO_ROOT / "edge" / "database" / "src" / "bin" / "devseed" / "client_menu.rs"
MANIFEST_OUT = REPO_ROOT / "seed" / "menu-manifest.json"
...
def main() -> int:
    ...
    argv = sys.argv[1:]
    if "--workbook" in argv:
        WORKBOOK = Path(argv[argv.index("--workbook") + 1]).resolve()
    if not WORKBOOK.exists():
        print(f"menu-to-seed: no workbook at {WORKBOOK}", file=sys.stderr)
        print("  pass --workbook <path> if the client's file lives elsewhere.", file=sys.stderr)
        return 1
    ...
    if missing_addons:
        print(
            f"menu-to-seed: the Read me names add-on rows that are not in the "
    ...
    lines.append("// @generated by scripts/menu-to-seed.py -- DO NOT EDIT BY HAND.")
    lines.append("//")
    ...
    lines.append("// here fails `client_menu_matches_the_generated_manifest` in devseed.")
    lines.append("//")
    ...
    lines.append("pub const CLIENT_CATEGORIES: &[(&str, i64, &[SeedItem])] = &[")
    ...
    manifest = {
        "generated_by": "scripts/menu-to-seed.py",
        "source": str(WORKBOOK.relative_to(REPO_ROOT)).replace("\\", "/"),
    ...
    print(f"menu-to-seed: {RUST_OUT.relative_to(REPO_ROOT)}")
    print(
