...
# NO VITE_KDS_LAN_URL. The KDS now derives the till's address from the address
# THIS PAGE WAS SERVED FROM (apps/kds/src/lib/lanConfig.ts), because the till
# is what serves it: the kitchen laptop opens http://<till>:5174/ and this
# machine opens http://localhost:5174/.
#
# Writing the host here is what made it go stale. Vite freezes a VITE_ var into
# the bundle at DEV-SERVER START, so a server started before the network moved,
# or a file written for a previous hotspot lease, served a dead address for
# ever -- and lanClient reconnects on every close, so it retried that address
# silently and never errored. docs/backlog.md records the live incident: a host
# that was never one of this machine's addresses was written here, printed back
# as an OK line, and passed every downstream check because Test-NetConnection
# succeeded against the WRONG host and nothing compared the two.
#
# Only the PORT is written, and only because the edge's listener is separate
# from the page's (Vite 5174, edge 9310) so it cannot be derived. Setting
# VITE_KDS_LAN_URL by hand still overrides everything -- that is the rollback.
$kdsEnvLines = @(
    "# Generated by scripts/dev-bootstrap.ps1. DO NOT COMMIT.",
    ...
    "# The till's HOST is derived from the address this page is served from and is",
    "# deliberately not written here. Set VITE_KDS_LAN_URL to override (rollback).",
    "VITE_KDS_LAN_PORT=$LanPort",
...
Write-Host "wrote $kdsEnvFile (no LAN host baked in -- the KDS reads the till's address from the page it was served from)" -ForegroundColor Cyan
Write-Host "  Open the kitchen screen from a second device at http://${lanIp}:5174/ and it will connect back to ${lanIp}:$LanPort." -ForegroundColor DarkGray
